...
import { DataSource, getDashboardSummary, getIssues, getPaymentMethods, getVolume } from "@/lib/api";
...
export default async function OverviewPage({ searchParams }: { searchParams: Promise<{ source?: string }> }) {
  ...
  const [summary, volume, methods, issues] = await Promise.all([getDashboardSummary(source), getVolume("7D", source), getPaymentMethods("30D", source), getIssues(source)]);
  return <>
    ...
    <section className="mt-6 rounded-xl border border-slate-200 bg-white shadow-card"><div className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><div><h2 className="font-semibold">Issues Requiring Attention</h2><p className="mt-1 text-sm text-slate-500">Only recorded alerts for {sourceLabel}</p></div><span className="text-sm font-semibold text-brand">{summary.open_alerts} open</span></div><div className="divide-y divide-slate-100">{issues.length ? issues.map(issue => <article className="flex gap-4 px-5 py-4" key={issue.id}><StatusBadge value={issue.severity}/><div><h3 className="text-sm font-semibold">{issue.title}</h3><p className="mt-1 text-sm text-slate-500">{issue.description}</p></div></article>) : <div className="px-5 py-10 text-center text-sm text-slate-500">No recorded alerts for this source.</div>}</div></section>
